<?php
	...
	foreach($chopped_search_text_array as $search_items){
		$search_sqli_statement .= "class_name LIKE '%".$search_items."%'"."\n"." numeric_class_name LIKE '%".$search_items."%'"."\n"." student_capacity LIKE '%".$search_items."%'"."\n"." session LIKE '%".str_replace(["/","-"],"-",$search_items)."%'";
	...
	if((isset($_GET["search"])) && (trim(strip_tags($_GET["search"])) !== "")){
		$select_class_table_lists = mysqli_query($connection_server, "SELECT * FROM sm_classes WHERE $search_sqli_statements LIMIT $page_pnum OFFSET ".((($current_page_no)-1)*$page_pnum));
		$select_all_class_table_lists = mysqli_query($connection_server, "SELECT * FROM sm_classes WHERE $search_sqli_statements");
	}else{
		$select_class_table_lists = mysqli_query($connection_server, "SELECT * FROM sm_classes WHERE school_id_number='".trim(strip_tags($_GET['id']))."' LIMIT $page_pnum OFFSET ".((($current_page_no)-1)*$page_pnum));
		$select_all_class_table_lists = mysqli_query($connection_server, "SELECT * FROM sm_classes WHERE school_id_number='".trim(strip_tags($_GET['id']))."'");
	}
	...
	if(isset($_POST["add-class"])){
		...
		if(!empty($class_name) && !empty($numeric_class_name) && !empty($student_capacity) && !empty($session) && !empty($school_id)){
			if(mysqli_num_rows(mysqli_query($connection_server, "SELECT * FROM sm_classes WHERE (school_id_number='$school_id' && numeric_class_name='$numeric_class_name' && session='$session')")) == 0){
				if(mysqli_query($connection_server, "INSERT INTO sm_classes (school_id_number, class_name, numeric_class_name, student_capacity, session) VALUES ('$school_id','$class_name','$numeric_class_name','$student_capacity','$session')") == true){
	...
	if(isset($_POST["update-class"])){
		...
		$current_session = mysqli_real_escape_string($connection_server, trim(strip_tags($_GET["session"])));
		$current_numeric_class_name = mysqli_real_escape_string($connection_server, trim(strip_tags($_GET["edit"])));
				
		$search_class_with_id_numeric = mysqli_query($connection_server, "SELECT * FROM sm_classes WHERE school_id_number='$school_id' && (numeric_class_name='$numeric_class_name' OR numeric_class_name='$current_numeric_class_name') && (session='$session' OR session='$current_session')");
		
		if(!empty($class_name) && !empty($numeric_class_name) && !empty($student_capacity) && !empty($school_id)){
			if(mysqli_num_rows($search_class_with_id_numeric) == 1){
				if(mysqli_query($connection_server, "UPDATE sm_classes SET class_name='$class_name', numeric_class_name='$numeric_class_name', student_capacity='$student_capacity', session='$session' WHERE (school_id_number='$school_id' && numeric_class_name='$current_numeric_class_name' && session='$current_session')") == true){
					$redirect_url = "/bc-admin.php?page=".strip_tags($_GET['page'])."&tab=true".$additional_back_tag;
				}
			}else{
	...
	if(isset($_POST["delete-class"])){
		...
		foreach($class_id as $index => $class_id_no){
			$numeric_class_name = mysqli_real_escape_string($connection_server, $class_id[$index]);
			...
			$delete_school_selected_class = mysqli_query($connection_server, "DELETE FROM sm_classes WHERE (school_id_number='$sch_id_number' && numeric_class_name='$numeric_class_name' && session='$session')");
		}
