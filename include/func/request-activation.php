...
$settings_query = mysqli_query($connection_server, "SELECT `_key`, `value` FROM sm_sms_settings WHERE `_key` IN ('paystack_public_key', 'bank_name', 'bank_account_name', 'bank_account_number')");
$settings = [];
while ($row = mysqli_fetch_assoc($settings_query)) {
    $settings[$row['_key']] = $row['value'];
}
