...
<?php if(strip_tags($_GET['tab']) == "true"){ ?>
    <?php
        if(mysqli_num_rows(mysqli_query($connection_server, "SELECT * FROM sm_hostels WHERE school_id_number='".trim(strip_tags($_GET['id']))."'")) > 0){
			$count_hostel_listed = mysqli_num_rows($select_all_hostel_table_lists);
    ?>
    <div class="container-box bg-2 mobile-width-100 system-width-100 mobile-margin-top-1 system-margin-top-1">
		...
		<div style="text-align: left;" class="scroll-box bg-2 mobile-width-96 system-width-96">
        	...
        	<form method="post">
        		...
        		<span class="color-7 mobile-font-size-16 system-font-size-18">Showing <?php echo ((($page_pnum*$current_page_no)-$page_pnum)+1); ?> to <?php echo ($page_pnum*$current_page_no); ?> of <?php echo $count_hostel_listed; ?> entries</span>
			...
			<?php } ?>
           	<form method="post" enctype="multipart/form-data">
           		<table class="table-tag-borderless mobile-font-size-12 system-font-size-14 mobile-margin-left-3 system-margin-left-2">
					...
					<?php
						if(mysqli_num_rows($select_all_hostel_table_lists) > 0){
							while(($hostel_details = mysqli_fetch_assoc($select_hostel_table_lists))){
								$hostel_view_link = str_replace('tab=true','tab='.$header_view_button,$_SERVER['REQUEST_URI'])."&view=".$hostel_details["id_number"];
								$hostel_edit_link = str_replace('tab=true','tab='.$header_add_button,$_SERVER['REQUEST_URI'])."&edit=".$hostel_details["id_number"];
								...
								$registered_students = mysqli_query($connection_server, "SELECT * FROM sm_hostel_list WHERE (school_id_number='".$mod_school_id["school_id_number"]."' && numeric_hostel_name='".$hostel_details["numeric_hostel_name"]."' && session='".$hostel_details["session"]."')");
								...
								if(($user_identifier_auth_id != "super_mod") && ($user_identifier_auth_id == "mod_adm") || ($user_identifier_auth_id == "adm_staff") && ($user_identifier_auth_id != "teacher") && ($user_identifier_auth_id != "stu_par") && ($user_identifier_auth_id != "stu")){
									$dcheck_button = '<td>
														<input type="checkbox" name="hostel_id[]" value="'.$hostel_details["id_number"].'" class="hostelChecked" />
														<input hidden type="text" name="school_id[]" value="'.$hostel_details["school_id_number"].'" />
													</td>';
									$action_button = '<td>
														<img onclick="return popUpAlert([``,``,`'.$hostel_edit_link.'`,``],[`View hostel Details`,``,`Edit Hostel`,``]);" src="imgfile/More.png" style="cursor: pointer;" class="onhover-bg-color-6 mobile-width-40 system-width-30" />
													</td>';
								}
								...
								echo '<tr>
									...
									<td>'.$hostel_details["hostel_name"].'</td>
           							<td>'.$hostel_details["hostel_type"].'</td>
									<td>'.checkIfEmpty($hostel_details["hostel_description"]).'</td>
									'.$action_button.'
									</tr>';
							}
						}
				...
				<button name="delete-hostel" type="submit" id="delHostel" style="display: none;" class="color-2 bg-3 mobile-font-size-14 system-font-size-16 mobile-padding-left-5 system-padding-left-2 mobile-padding-right-5 system-padding-right-2 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-4 system-margin-left-2 mobile-margin-right-1 system-margin-right-1">
           			Delete Hostel
           		</button><br>
				<?php } ?>
			</form>
		...
		<script>
			...
			function checkALL(){
				var allBoxToChecked = document.getElementsByClassName("hostelChecked");
				if(document.getElementsByClassName("hostelChecked")[0].checked != true){
					for(i = 0; i < allBoxToChecked.length; i++){
						...
						document.getElementsByClassName("hostelChecked")[i].checked = "checked";
					}
				}else{
					for(i = 0; i < allBoxToChecked.length; i++){
						if(document.getElementsByClassName("checkALL")[0].checked == true){
						...
						document.getElementsByClassName("hostelChecked")[i].checked = false;
					}
				}
			}
			...
			function deleteItems(){
				...
				if(checkBoxCount == 1){
					if(confirm("Are you sure you want to delete this hostel?")){
						document.getElementById("delHostel").click();
					}else{
						alert("Operation Cancelled");
					}
				}else{
					if(checkBoxCount > 1){
						//alert("You cannot pick more than one hostel");
						if(confirm("Are you sure you want to delete this hostel?")){
							document.getElementById("delHostel").click();
						}else{
							alert("Operation Cancelled");
...
<?php
    if(($user_identifier_auth_id != "super_mod") && ($user_identifier_auth_id == "mod_adm") || ($user_identifier_auth_id == "adm_staff") && ($user_identifier_auth_id != "teacher") && ($user_identifier_auth_id != "stu_par") && ($user_identifier_auth_id != "stu")){
?>
<?php if(strip_tags($_GET['tab']) == 'add_hostel'){ ?>
    <div class="container-box bg-2 mobile-width-100 system-width-100 mobile-margin-top-1 system-margin-top-1">
		<center>
			<?php
				$edit_hostel_checkmate = mysqli_query($connection_server, "SELECT * FROM sm_hostels WHERE (school_id_number='".trim(strip_tags($_GET['id']))."' && id_number='".trim(strip_tags($_GET['edit']))."')");
				if((isset($_GET['edit'])) && (trim(strip_tags($_GET['edit'])) !== "") && (mysqli_num_rows($edit_hostel_checkmate) == 1)){
					if(mysqli_num_rows($edit_hostel_checkmate) == 1){
						$edit_hostel_detail = mysqli_fetch_array($edit_hostel_checkmate);
						$edit_hostel_moderator_detail = mysqli_fetch_array(mysqli_query($connection_server, "SELECT * FROM sm_moderators WHERE school_id_number='".trim(strip_tags($_GET['edit']))."' LIMIT 1"));
					}
				}
			?>
			<?php if(((isset($_GET['edit'])) && (trim(strip_tags($_GET['edit'])) !== "") && (mysqli_num_rows($edit_hostel_checkmate) == 1)) || ((!isset($_GET['edit'])) && (trim(strip_tags($_GET['edit'])) == "") && (isset($_GET['tab'])))){ ?>
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
				    <input name="hostel-name" value="<?php echo $edit_hostel_detail['hostel_name']; ?>" type="text" placeholder="" class="form-input" required/>
                    <span class="form-span mobile-font-size-12 system-font-size-14">Hostel Name*</span>
                </div>

                <div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
				    <input name="hostel-type" value="<?php echo $edit_hostel_detail['hostel_type']; ?>" type="text" placeholder="" class="form-input" required/>
                    <span class="form-span mobile-font-size-12 system-font-size-14">Hostel Type*</span>
                </div>

                <div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
				    <input name="hostel-desc" value="<?php echo $edit_hostel_detail['hostel_description']; ?>" type="text" placeholder="" class="form-input"/>
                    <span class="form-span mobile-font-size-12 system-font-size-14">Description</span>
                </div>
                
				<?php if((!isset($_GET['edit'])) || (trim(strip_tags($_GET['edit'])) == "") || (mysqli_num_rows($edit_hostel_checkmate) < 1)){ ?>
                <button name="add-hostel" type="submit" class="button-box color-2 bg-4 onhover-bg-color-7 mobile-font-size-14 system-font-size-16 mobile-width-93 system-width-46 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-5 system-margin-left-3 mobile-margin-right-1 system-margin-right-1">
				    ADD HOSTEL
				</button>
				<?php }else{ ?>
				<button name="update-hostel" type="submit" class="button-box color-2 bg-4 onhover-bg-color-7 mobile-font-size-14 system-font-size-16 mobile-width-93 system-width-46 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-5 system-margin-left-3 mobile-margin-right-1 system-margin-right-1">
				    UPDATE HOSTEL
				</button>
				<?php } ?>
            </form>
