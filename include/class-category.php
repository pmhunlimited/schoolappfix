...
<?php if(strip_tags($_GET['tab']) == "true"){ ?>
    <?php
        if(mysqli_num_rows(mysqli_query($connection_server, "SELECT * FROM sm_class_category WHERE school_id_number='".trim(strip_tags($_GET['id']))."'")) > 0){
        ...
        <div style="text-align: left;" class="scroll-box bg-2 mobile-width-96 system-width-96">
            ...
            <form method="post" enctype="multipart/form-data">
                <table class="table-tag-borderless mobile-font-size-12 system-font-size-14 mobile-margin-left-3 system-margin-left-2">
                    <tr>
                        <td>Tick</td>
                        <td class="mobile-width-10 system-width-10">Category</td>
                    ...
                    <?php
                        if(mysqli_num_rows($select_all_class_table_lists) > 0){
                            while(($class_details = mysqli_fetch_assoc($select_class_table_lists))){
                                $class_view_link = str_replace('tab=true','tab='.$header_view_button,$_SERVER['REQUEST_URI'])."&view=".$class_details["numeric_class_category_name"];
                                $class_edit_link = str_replace('tab=true','tab='.$header_add_button,$_SERVER['REQUEST_URI'])."&edit=".$class_details["numeric_class_category_name"];
                                ...
                                echo '<tr>
                                    <td>
                                        <input type="checkbox" name="class_category_id[]" value="'.$class_details["numeric_class_category_name"].'" class="classChecked" />
                                    ...
                                    <td>'.$class_details["class_category_name"].'</td>
                                    <td>
                                        <img onclick="return popUpAlert([``,``,`'.$class_edit_link.'`,``],[`View Class Details`,``,`Edit Class Category`,``]);" src="imgfile/More.png" style="cursor: pointer;" class="onhover-bg-color-6 mobile-width-40 system-width-30" />
                                    </td>
                                    </tr>';
                            }
                        }
...
<?php if(strip_tags($_GET['tab']) == 'add_class_category'){ ?>
    <div class="container-box bg-2 mobile-width-100 system-width-100 mobile-margin-top-1 system-margin-top-1">
        <center>
            <?php
                $edit_class_checkmate = mysqli_query($connection_server, "SELECT * FROM sm_class_category WHERE (school_id_number='".trim(strip_tags($_GET['id']))."' && numeric_class_category_name='".trim(strip_tags($_GET['edit']))."')");
                if((isset($_GET['edit'])) && (trim(strip_tags($_GET['edit'])) !== "") && (mysqli_num_rows($edit_class_checkmate) == 1)){
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
                    <input name="class-category-name" value="<?php echo $edit_class_detail['class_category_name']; ?>" type="text" placeholder="" class="form-input" required/>
                    <span class="form-span mobile-font-size-12 system-font-size-14">Class Category Name*</span>
                </div>
                ...
                <button name="add-class-category" style="float: right; clear: right;" type="submit" class="button-box color-2 bg-4 onhover-bg-color-7 mobile-font-size-14 system-font-size-16 mobile-width-93 system-width-46 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-1 system-margin-left-3 mobile-margin-right-2 system-margin-right-1">
                    ADD CLASS CATEGORY
                </button>
                <?php }else{ ?>
                <button name="update-class-category" style="float: right; clear: right;" type="submit" class="button-box color-2 bg-4 onhover-bg-color-7 mobile-font-size-14 system-font-size-16 mobile-width-93 system-width-46 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-1 system-margin-left-3 mobile-margin-right-2 system-margin-right-1">
