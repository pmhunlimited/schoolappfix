...
<?php if(strip_tags($_GET['tab']) == "true"){ ?>
    <?php
        if(mysqli_num_rows(mysqli_query($connection_server, "SELECT * FROM sm_notices WHERE school_id_number='".trim(strip_tags($_GET['id']))."' ".$user_notice_statement_auth)) > 0){
			$count_notice_listed = mysqli_num_rows($select_all_notice_table_lists);
    ?>
    <div class="container-box bg-2 mobile-width-100 system-width-100 mobile-margin-top-1 system-margin-top-1">
		...
		<div style="text-align: left;" class="scroll-box bg-2 mobile-width-96 system-width-96">
        	...
        	<form method="post">
        		...
        		<span class="color-7 mobile-font-size-16 system-font-size-18">Showing <?php echo ((($page_pnum*$current_page_no)-$page_pnum)+1); ?> to <?php echo ($page_pnum*$current_page_no); ?> of <?php echo $count_notice_listed; ?> entries</span>
           	...
           	<form method="post" enctype="multipart/form-data">
           		<table class="table-tag-borderless mobile-font-size-12 system-font-size-14 mobile-margin-left-3 system-margin-left-2">
           			<tr>
						...
						<td>Tick</td>
						<?php } ?>
           				<td class="mobile-width-10 system-width-10">Notice</td>
						<td>Notice Title</td>
           				<td>Notice Comment</td>
           				<td>Notice Start Date</td>
					...
					<?php
						function noticeClassName($class_info,$school_id){
						...
						if(mysqli_num_rows($select_all_notice_table_lists) > 0){
							while(($notice_details = mysqli_fetch_assoc($select_notice_table_lists))){
								$notice_view_link = str_replace('tab=true','tab='.$header_view_button,$_SERVER['REQUEST_URI'])."&view=".$notice_details["notice_id"];
								$notice_edit_link = str_replace('tab=true','tab='.$header_add_button,$_SERVER['REQUEST_URI'])."&edit=".$notice_details["notice_id"];
								
								if(($user_identifier_auth_id != "super_mod") && ($user_identifier_auth_id == "mod_adm") || ($user_identifier_auth_id == "adm_staff") && ($user_identifier_auth_id != "teacher") && ($user_identifier_auth_id != "stu_par") && ($user_identifier_auth_id != "stu")){
									$dcheck_button = '<td>
														<input type="checkbox" name="notice_id[]" value="'.$notice_details["notice_id"].'" class="noticeChecked" />
														<input hidden type="text" name="school_id[]" value="'.$notice_details["school_id_number"].'" />
													</td>';
									$action_button = '<td>'.ucwords($notice_details["notice_for"]).'</td>
													<td>'.ucwords(noticeClassName($notice_details["numeric_class_name"], $notice_details["school_id_number"])).'</td>
													<td>
														<img onclick="return popUpAlert([``,``,`'.$notice_edit_link.'`,``],[`View`,``,`Edit`,``]);" src="imgfile/More.png" style="cursor: pointer;" class="onhover-bg-color-6 mobile-width-40 system-width-30" />
													</td>';
								}

								echo '<tr>
									'.$dcheck_button.'
									<td><img style="position: relative; margin: -1.5% 0 0 -2%; background-color: #50C878; padding: 15%; border-radius: 15px;" src="imgfile/notice.png" class="mobile-width-60 system-width-30" /></td>
									<td>'.$notice_details["notice_title"].'
									<td>'.checkIfEmpty($notice_details["notice_comment"]).'
           							<td>'.str_replace("-","/",$notice_details["start_date"]).'</td>
									<td>'.str_replace("-","/",$notice_details["end_date"]).'</td>
									'.$action_button.'
									</tr>';
							}
						}
				...
				<button name="delete-notice" type="submit" id="delNotice" style="display: none;" class="color-2 bg-3 mobile-font-size-14 system-font-size-16 mobile-padding-left-5 system-padding-left-2 mobile-padding-right-5 system-padding-right-2 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-4 system-margin-left-2 mobile-margin-right-1 system-margin-right-1">
           			Delete notice
           		</button><br>
				<?php } ?>
			</form>
		...
		<script>
			...
			function checkALL(){
				var allBoxToChecked = document.getElementsByClassName("noticeChecked");
				if(document.getElementsByClassName("noticeChecked")[0].checked != true){
					for(i = 0; i < allBoxToChecked.length; i++){
						...
						document.getElementsByClassName("noticeChecked")[i].checked = "checked";
					}
				}else{
					for(i = 0; i < allBoxToChecked.length; i++){
						if(document.getElementsByClassName("checkALL")[0].checked == true){
						...
						document.getElementsByClassName("noticeChecked")[i].checked = false;
					}
				}
			}
			...
			function deleteItems(){
				...
				if(checkBoxCount == 1){
					if(confirm("Are you sure you want to delete this notice?")){
						document.getElementById("delNotice").click();
					}else{
						alert("Operation Cancelled");
					}
				}else{
					if(checkBoxCount > 1){
						//alert("You cannot pick more than one notice");
						if(confirm("Are you sure you want to delete this notice?")){
							document.getElementById("delNotice").click();
						}else{
							alert("Operation Cancelled");
...
<?php
    if(($user_identifier_auth_id != "super_mod") && ($user_identifier_auth_id == "mod_adm") || ($user_identifier_auth_id == "adm_staff") && ($user_identifier_auth_id != "teacher") && ($user_identifier_auth_id != "stu_par") && ($user_identifier_auth_id != "stu")){
?>
<?php if(strip_tags($_GET['tab']) == 'add_notice'){ ?>
    <div class="container-box bg-2 mobile-width-100 system-width-100 mobile-margin-top-1 system-margin-top-1">
		<center>
			<?php
				$edit_notice_checkmate = mysqli_query($connection_server, "SELECT * FROM sm_notices WHERE (school_id_number='".trim(strip_tags($_GET['id']))."' && notice_id='".trim(strip_tags($_GET['edit']))."')");
				if((isset($_GET['edit'])) && (trim(strip_tags($_GET['edit'])) !== "") && (mysqli_num_rows($edit_notice_checkmate) == 1)){
					if(mysqli_num_rows($edit_notice_checkmate) == 1){
						$edit_notice_detail = mysqli_fetch_array($edit_notice_checkmate);
						$edit_notice_moderator_detail = mysqli_fetch_array(mysqli_query($connection_server, "SELECT * FROM sm_moderators WHERE school_id_number='".trim(strip_tags($_GET['edit']))."' LIMIT 1"));
					}
				}
			?>
			<?php if(((isset($_GET['edit'])) && (trim(strip_tags($_GET['edit'])) !== "") && (mysqli_num_rows($edit_notice_checkmate) == 1)) || ((!isset($_GET['edit'])) && (trim(strip_tags($_GET['edit'])) == "") && (isset($_GET['tab'])))){ ?>
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
				    <input name="notice-title" value="<?php echo $edit_notice_detail['notice_title']; ?>" type="text" placeholder="" class="form-input" required/>
                    <span class="form-span mobile-font-size-12 system-font-size-14">Notice Title*</span>
                </div>

                <div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
				    <input name="notice-comment" value="<?php echo $edit_notice_detail['notice_comment']; ?>" type="text" placeholder="" class="form-input" />
                    <span class="form-span mobile-font-size-12 system-font-size-14">Notice Comment</span>
                </div>
				
				<div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
					<input name="start-date" type="date" min="<?php echo date('Y-m-d'); ?>" value="<?php echo $edit_notice_detail['start_date']; ?>" class="form-input" required/>
					<span class="form-span mobile-font-size-12 system-font-size-14">Start Date*</span>
				</div>
				
				<div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
					<input name="end-date" type="date" min="<?php echo date('Y-m-d'); ?>" value="<?php echo $edit_notice_detail['end_date']; ?>"  class="form-input" required/>
					<span class="form-span mobile-font-size-12 system-font-size-14">End Date*</span>
				</div>

				<div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
					<select name="notice-for" class="form-select" required>
						<option <?php if($edit_notice_detail['notice_for'] == "all"){ echo 'selected'; } ?> value="all">All</option>
						<option <?php if($edit_notice_detail['notice_for'] == "teacher"){ echo 'selected'; } ?> value="teacher">Teacher</option>
						<option <?php if($edit_notice_detail['notice_for'] == "student"){ echo 'selected'; } ?> value="student">Student</option>
						<option <?php if($edit_notice_detail['notice_for'] == "parent"){ echo 'selected'; } ?> value="parent">Parent</option>
						<option <?php if($edit_notice_detail['notice_for'] == "admin"){ echo 'selected'; } ?> value="admin">Admin Staff</option>
					</select>
					<span class="form-span mobile-font-size-12 system-font-size-14">Notice For</span>
				</div>
				
				<div class="form-group mobile-width-90 system-width-45 mobile-margin-top-2 system-margin-top-2 mobile-margin-bottom-2 system-margin-bottom-2 mobile-margin-left-2 system-margin-left-2 mobile-margin-right-2 system-margin-right-2">
					<select name="numeric-class" id="find-exam-class-session" class="form-select" required>
						...
						<?php
							...
							if(mysqli_num_rows($select_classes_detail_using_id) > 0){
								while($classes_details = mysqli_fetch_assoc($select_classes_detail_using_id)){
									if($classes_details["numeric_class_name"] == $edit_notice_detail['numeric_class_name']){
										$selected = "selected";
