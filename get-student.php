<?php session_start(); error_reporting(0);
	...
	$get_student_detail = mysqli_query($connection_server,"SELECT * FROM sm_class_list WHERE school_id_number='".$get_statement_student["sch_no"]."' && numeric_class_name='".$get_statement_student["class_id_no"]."' && session='".$get_statement_student["session"]."'");
	    $studentArray = array();
	    if(mysqli_num_rows($get_student_detail) >= 1){
	        while($stu_details = mysqli_fetch_assoc($get_student_detail)){
